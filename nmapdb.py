...
# This version is slightly modified to support updating from multiple files that might contain the same IP(prevents unique constaraints error) 
#edited by roy haroush
import sys
import os
import getopt
    ...
    for fname in args:
        ...
        for host in doc.getElementsByTagName("host"):
            ...
            for port in ports:
                ...
                if nodb_flag == false:
                    try:
                        cursor.execute("INSERT or REPLACE INTO ports VALUES (?, ?, ?, ?, ?, ?, ?)", (ip, pn, protocol, port_name, state, service_str, info_str))
                    except sqlite.IntegrityError, msg:
